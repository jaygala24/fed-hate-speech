...
parser.add_argument("--batch_size", type=int, default=64, help="batch size for fine-tuning the model")
parser.add_argument(
    "--seed", type=int, default=42, help="random seed for deterministic behaviour and reproducibility",
)
parser.add_argument(
...
parser.add_argument(
    "--E", "--epochs", type=int, default=1, help="number of training epochs on local dataset for each round",
)
parser.add_argument(
    "--algorithm",
    type=str,
...
parser.add_argument("--es_round_start", type=int, default=100, help="round to start early stopping from")
parser.add_argument("--save", type=str, default="exp", help="experiment name")
parser.add_argument(
    "--wandb", action="store_true", default=False, help="use wandb for tracking results",
)
parser.add_argument(
    "--wandb_proj_name", type=str, default="hate-speech-comb", help="provide a project name",
)
parser.add_argument("--wandb_run_name", type=str, default="exp", help="provide a run name")
parser.add_argument("--wandb_run_notes", type=str, default="", help="provide notes for a run if any")
...
def main():
    ...
    new_categories = list(raw_data["train"]["category"].unique())
    ...
    if args.class_weights:
        class_weights = class_weight.compute_class_weight(class_weight="balanced", classes=classes, y=class_array)
    else:
        class_weights = class_weight.compute_class_weight(class_weight="balanced", classes=classes, y=classes)
    ...
    if args.wandb:
        run = wandb.init(
            name=args.wandb_run_name, project=args.wandb_proj_name, notes=args.wandb_run_notes, config=config_dict,
        )
    ...
    # load the best checkpoint for the current run on the wandb if enabled
    if args.wandb:
...
class FedTrainer:
    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.rounds + 1):
            ...
            for client_idx, epoch in zip(client_idxs, client_epoch_list):
                client = Client(self.args, epoch, self.train_data, self.local_data_idxs[client_idx])
            ...
            if val_metrics["valid/f1_score"] > best_f1_loss:
                ...
                patience = 0
                torch.save(self.model.state_dict(), os.path.join(self.args.save, "pytorch_model.bin"))
            else:
                # start early stopping after the `x` rounds
                if round_idx > args.es_round_start:
                    patience += 1
                    logging.info(f"Early stopping counter {patience} out of {self.args.es_patience}")
                    if patience == self.args.es_patience:
                        self.model.load_state_dict(
                            torch.load(
                                os.path.join(self.args.save, "pytorch_model.bin"), map_location=torch.device("cuda"),
                            )
                        )
            ...
            if round_idx == self.args.rounds:
                self.model.load_state_dict(
                    torch.load(os.path.join(self.args.save, "pytorch_model.bin"), map_location=torch.device("cuda"),)
                )
                test_metrics, test_loss = self.eval(stage="test")
                logging.info(
                    f"FINAL TESTING\n... \tTest Loss: {round(test_loss, 3)}... Test Accuracy: {test_metrics['test/accuracy']}...  "
    ...
    def eval(self, stage="valid"):
        ...
        criterion = nn.CrossEntropyLoss(weight=torch.FloatTensor(self.args.class_weights_array).cuda())
...
class Client:
    def __init__(self, args, epochs, data, idxs):
        ...
        self.dataloader = DataLoader(
            CustomDataset(data, idxs), batch_size=self.args.batch_size, shuffle=True, collate_fn=pad_collate,
        )

    def train(self, model):
        criterion = nn.CrossEntropyLoss(weight=torch.FloatTensor(self.args.class_weights_array).cuda(),)
        if args.model_type == "lstm":
            optimizer = torch.optim.Adam(model.parameters(), lr=self.args.client_lr, weight_decay=1.2e-6)
        else:
